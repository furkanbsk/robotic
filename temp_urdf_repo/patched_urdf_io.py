...
class URDFLoader():
    ...
    def __init__(self, file, visualsOnly=True, meshPathRemove='', meshExt=None):
        ...
        for link in links:
            ...
            # PATCHED: use enumeration for unique names
            for i, visual in enumerate(link.findall('visual')):
                self.add_shape(visual, f'{link_name}_vis_{i}', f_link, isVisual=True)

            # collision shape
            if useCollisionShapes:
                for i, collision in enumerate(link.findall('collision')):
                    f_shape = self.add_shape(collision, f'{link_name}_col_{i}', f_link, isVisual=False)
        ...
        for joint in joints:
            ...
            f_joint = self.C.addFrame(f'{joint_name}_joint_frame')
            ...
            f_mimic = None
            if elem is not None:
                f_mimic = self.C.getFrame(elem.attrib['joint'] + '_joint_frame')
                if f_mimic is None:
    ...
    def add_shape(self, shape, shape_name, f_link, isVisual):
        ...
        if elem is not None:
            ...
            f_shape.setMeshFile(filename, scale) # This will check file existence in ry C++
