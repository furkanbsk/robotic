...
case ${lib} in
	...
	cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_INSTALL_PREFIX=${pre} -DBUILD_EXAMPLES=OFF -DBUILD_TESTS=OFF ${lib} -B ${lib}/build
	make -C ${lib}/build install
	;;
	...
	env CFLAGS="-fPIC" cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_INSTALL_PREFIX=${pre} -DBUILD_SHARED_LIBS=ON ${lib} -DENABLE_DOUBLE_PRECISION=ON -B ${lib}/build
	make -C ${lib}/build install
	env CFLAGS="-fPIC" cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_INSTALL_PREFIX=${pre} -DBUILD_SHARED_LIBS=OFF ${lib} -DENABLE_DOUBLE_PRECISION=ON -B ${lib}/build  #compile shared AND static versions!
	make -C ${lib}/build install
	;;

    fcl)
	git clone --single-branch -b fcl-0.5 https://github.com/flexible-collision-library/fcl.git
        cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_INSTALL_PREFIX=${pre} -DFCL_STATIC_LIBRARY=ON -DFCL_BUILD_TESTS=OFF -DFCL_WITH_OCTOMAP=OFF -DCMAKE_CXX_FLAGS="-Wno-deprecated-copy -Wno-class-memaccess -Wno-maybe-uninitialized" ${lib} -B ${lib}/build
	make -C ${lib}/build install
	;;
